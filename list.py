...
for url in page_urls:
    ...
    for item in url_list:
        new_driver.get(item)
        desc = new_driver.find_elements(By.XPATH,'//span[@class="a-size-large product-title-word-break"]')
        asin = new_driver.find_elements(By.XPATH,'//span[@class="a-size-large product-title-word-break"]')
        product_desc = new_driver.find_elements(By.XPATH,'//span[@class="a-size-large product-title-word-break"]')
        manfacture = new_driver.find_elements(By.XPATH,'//span[@class="a-size-large product-title-word-break"]')
        for desc in desc:
            description.append(desc.text)
        for desc in asin:
            asin.append(desc.text)
        for desc in product_desc:
            product_desc.append(desc.text)
        for desc in manfacture:
            manfacture.append(desc.text)

data = {
    'Description': description,
    'Asin': asin,
    'product_descrption': product_desc,
    'Manfacture': manfacture,
}

df = pd.DataFrame(data)
output_path = 'items.csv'  # Replace with the desired output file path
